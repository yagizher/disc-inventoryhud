        ...
        if (itemData !== undefined) {
            ...
            if (!itemData.unique) {
                if (itemData.stackable) {
                    $('.tooltip-div').find('.tooltip-uniqueness').html("Eşsiz değil - Maks limit (" + itemData.max + ")");
                } else {
                    $('.tooltip-div').find('.tooltip-uniqueness').html("Eşsiz değil - Çoğaltılamaz");
                }
            } else {
                $('.tooltip-div').find('.tooltip-uniqueness').html("Eşsiz (" + itemData.max + ")");
            }

            if (itemData.description !== undefined) {
                $('.tooltip-div').find('.tooltip-desc').html('Açıklama: ' + itemData.description);
            } else {
                $('.tooltip-div').find('.tooltip-desc').html("Bu eşyanın açıklaması yok");
            }

            if (itemData.weight !== undefined) {
                $('.tooltip-div').find('.tooltip-weight').html('Limit: ' + itemData.weight * itemData.qty);
            } else {
            ...
            if (itemData.staticMeta !== undefined || itemData.staticMeta !== "") {
                if (itemData.type === 1) {
                    $('.tooltip-div').find('.tooltip-meta').append('<div class="meta-entry"><div class="meta-key">Kayıtlı Sahip</div> : <div class="meta-val">' + itemData.staticMeta.owner + '</div></div>');
                } else if (itemData.itemId === 'license') {
                    $('.tooltip-div').find('.tooltip-meta').append('<div class="meta-entry"><div class="meta-key">İsim</div> : <div class="meta-val">' + itemData.staticMeta.name + '</div></div>');
                    $('.tooltip-div').find('.tooltip-meta').append('<div class="meta-entry"><div class="meta-key">Neden</div> : <div class="meta-val">' + itemData.staticMeta.issuedDate + '</div></div>');
                    $('.tooltip-div').find('.tooltip-meta').append('<div class="meta-entry"><div class="meta-key">Boy</div> : <div class="meta-val">' + itemData.staticMeta.height + '</div></div>');
                    $('.tooltip-div').find('.tooltip-meta').append('<div class="meta-entry"><div class="meta-key">Doğum Günü</div> : <div class="meta-val">' + itemData.staticMeta.dob + '</div></div>');
                    $('.tooltip-div').find('.tooltip-meta').append('<div class="meta-entry"><div class="meta-key">Numara</div> : <div class="meta-val">' + itemData.staticMeta.phone + '</div></div>');
                    $('.tooltip-div').find('.tooltip-meta').append('<div class="meta-entry"><div class="meta-key">Kimlik ID</div> : <div class="meta-val">' + itemData.staticMeta.id + '-' + itemData.staticMeta.user + '</div></div>');

                    if (itemData.staticMeta.endorsements !== undefined) {
                        $('.tooltip-div').find('.tooltip-meta').append('<div class="meta-entry"><div class="meta-key">Onay</div> : <div class="meta-val">' + itemData.staticMeta.endorsements + '</div></div>');
                    }
                } else if (itemData.itemId === 'gold') {
                    $('.tooltip-div').find('.tooltip-meta').append('<div class="meta-entry"><div class="meta-key"></div> : <div class="meta-val">Bu bar los santos devleti tarafından onaylıdır.</div></div>');
                }
            } else {
                $('.tooltip-div').find('.tooltip-meta').html("Bu eşya hakkında bir bilgi yok.");
            }
            $('.tooltip-div').show();
        }
    });
...
function AttemptDropInEmptySlot(origin, destination, moveQty) {
    ...
    if (result === -1) {
        ...
        if (moveQty === item.qty) {
            ...
            InventoryLog('Slot guncelleniyor ' + item.qty + ' ' + item.label + ' ' + ' suradan ' + origin.data('invOwner') + ' suraya ' + origin.data('slot') + ' suraya ' + destination.parent().data('invOwner') + ' suraya ' + item.slot);
            ...
            InventoryLog('Boş: Slot guncelleniyor ' + moveQty + ' ' + item.label + ' suradan ' + origin.data('invOwner') + ' suraya ' + item.slot + ' suraya ve ' + destination.parent().data('invOwner') + ' suraya ' + item2.slot);
        ...
        if (result === 1) {
            ...
            InventoryLog("Envanter sahibi hedef bilinmiyor.");
        }
    }
}
    ...
    if ($('#action-bar').is(':visible')) {
        ...
        for (let i = 0; i < 5; i++) {
            ...
            if (items[i] != null) {
                ...
                $(`.slot-${i}`).find('.item-name').html('BOS');
        ...
        for (let i = 0; i < 5; i++) {
            if (items[i] != null) {
                ...
                $('#action-bar').append(`<div class="slot slot-${i}" data-empty="true"><div class="item"><div class="item-count"></div><div class="item-name">BOS</div><div class="item-keybind">${i + 1}</div></div></div>`);
                $(`.slot-${i}`).find('.item').css('background-image', 'none');
            }
        }
